...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Kraken.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Krakend.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeKrakenamount=<n> " + _("Keep N Kraken anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Kraken is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Kraken is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Kraken version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeKrakenAmount = GetArg("-anonymizeKrakenamount", 0);
    if(nAnonymizeKrakenAmount > 999999) nAnonymizeKrakenAmount = 999999;
    if(nAnonymizeKrakenAmount < 2) nAnonymizeKrakenAmount = 2;
    ...
    LogPrintf("Anonymize Kraken Amount %d\n", nAnonymizeKrakenAmount);
